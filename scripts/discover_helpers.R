# Shared helpers for discover_packages.R, sync_packages.R, and add_package.R.
# Pure-function file: source this from any script that needs the helpers.
...
write_pkg <- function(entry, dir) {
  ...
  # JSON `null` round-trips through read_json as R NULL; write_json then
  # serialises NULL list elements as `{}`. Coerce back to NA on the scalar
  # fields so na = "null" re-emits proper JSON null.
  scalar_fields <- c("name", "title", "description", "repo_url", "pkdown_url",
                     "bug_reports_url", "logo_url", "last_updated")
  for (f in scalar_fields) {
    if (is.null(merged[[f]])) merged[[f]] <- NA_character_
  }
