...
# Render one bash block for a single author's PR. `p` is a list with
# (slug, name, github, packages). Used by both the discovery and prune scripts.
emit_pr_block <- function(p) {
  branch <- sprintf("directory/%s", p$slug)
  staged_files <- file.path(
    "data/packages_pending",
    p$slug,
    paste0(p$packages, ".json")
  )
  target_files <- file.path("data/packages", paste0(p$packages, ".json"))
  pkg_list_md <- paste0("- `", p$packages, "`", collapse = "\n")
  title <- sprintf(
    "Add %d package%s by %s",
    length(p$packages),
    if (length(p$packages) == 1) "" else "s",
    p$name
  )
  ...
  copy_lines <- paste(
    sprintf("cp %s %s", shQuote(staged_files), shQuote(target_files)),
    collapse = "\n"
  )
  sprintf(
    "# --- %s (@%s): %d package(s) ---\ngit fetch upstream main\ngit checkout -b %s upstream/main\nmkdir -p data/packages\n%s\ngit add %s\ngit commit -m %s\ngit push -u origin %s\ngh pr create --repo rladies/awesome-rladies-creations --base main --head drmowinckels:%s --title %s --body %s\n\n",
    p$name,
    p$github,
    length(p$packages),
    branch,
    copy_lines,
    paste(shQuote(target_files), collapse = " "),
    shQuote(title),
    branch,
    branch,
    shQuote(title),
...
# Write the full _open_prs.sh script for a given plan (list of author lists).
# Source attribution in the header tells future-readers which generator
# produced this file.
write_open_prs_script <- function(plan, path, source_label) {
  writeLines(
    c(
      "#!/usr/bin/env bash",
      sprintf("# Generated by scripts/%s.", source_label),
      "#",
      "# Review staged files in data/packages_pending/<slug>/ before running.",
      "# Comment out or delete blocks for authors you don't want to PR.",
      "# Each block starts from main, so blocks are independent.",
      "#",
      "# Safety: refuses to run with a dirty working tree.",
      "set -euo pipefail",
      "",
      ...
      "fi",
      "",
      vapply(plan, emit_pr_block, character(1))
    ),
    path
  )
  Sys.chmod(path, "755")
